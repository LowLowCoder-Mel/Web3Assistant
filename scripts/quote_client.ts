...
async function testSynapse() {
  const inputAmount = parseUnits('10', 18).toString();
  const protocol = Protocol.Synapse;
  const chainId = ChainId.Ethereum;
  const inputToken = '0x6B175474E89094C44Da98b954EedeAC495271d0F';
  const outputToken = '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48';
  const poolAddress = '0x1116898DdA4015eD8dDefb84b6e8Bc24528Af2d8';
  const query: QuoteParam = {
    protocol,
    inputAmount,
    inputToken,
...
async function testDodo() {
  ...
  const chainId = ChainId.Ethereum;
  const inputToken = '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2';
  const outputToken = '0xA0b73E1Ff0B80914AB6fe0444E65848C4C34450b';
